...
SwimNode::SwimNode(int t, double x, double y, double z)
    : tag(t), xLoc(x), yLoc(y), zLoc(z)
{
}

int SwimMaterial::numTagND = 0;
int SwimMaterial::numTagUni = 0;

SwimMaterial::SwimMaterial()
    : writtenUniaxial(false), writtenND(false)
{
}

int SwimMaterial::getNewTag()
{
...
int Concrete::writeNDJSON(json_t *ndMaterials)
{
  if (this->writtenND == false)
  {
    ...
    json_object_set(obj, "beta", json_real(beta));
    json_object_set(obj, "Ap", json_real(Ap));
    json_object_set(obj, "An", json_real(An));
    json_object_set(obj, "Bn", json_real(Bn));
...
{
    longitudinalRebar = json_object_get(obj, "longitudinalRebar");
    transverseRebar = json_object_get(obj, "transverseRebar");
    longitudinalBoundaryElementRebar = json_object_get(obj, "longitudinalBoundaryElementRebar");
    transverseBoundaryElementRebar = json_object_get(obj, "transverseBoundaryElementRebar");


    const char *key;
    json_t *value;

    json_object_foreach(longitudinalRebar, key, value) {
        const string trueKey(key);
        string trueValue;
        if(json_is_string(value))
            trueValue = string(json_string_value(value));
        else if(json_is_real(value))
            trueValue = to_string(json_real_value(value));
        else
            trueValue = to_string(json_integer_value(value));

        longitudinalRebar_map.insert(std::pair<string, string>(trueKey, trueValue));
    }
    json_object_foreach(transverseRebar, key, value) {
        const string trueKey(key);
        string trueValue;
        if(json_is_string(value))
            trueValue = string(json_string_value(value));
        ...
        transverseRebar_map.insert(std::pair<string, string>(trueKey, trueValue));
    }
    json_object_foreach(longitudinalBoundaryElementRebar, key, value) {
        const string trueKey(key);
        string trueValue;
        if(json_is_string(value))
            trueValue = string(json_string_value(value));
        ...
        longitudinalBoundaryElementRebar_map.insert(std::pair<string, string>(trueKey, trueValue));
    }
    json_object_foreach(transverseBoundaryElementRebar, key, value) {
        const string trueKey(key);
        string trueValue;
        if(json_is_string(value))
            trueValue = string(json_string_value(value));
        ...
        transverseBoundaryElementRebar_map.insert(std::pair<string, string>(trueKey, trueValue));
    }
...
int ConcreteRectangularWallSection::writeNDJSON(json_t *elements, json_t *nodes, json_t *properties, json_t *nodeMapping, string cline1, string cline2, string floor1, string floor2)
{
  ...
  SwimMaterial *theLR_Material = theModel->getMaterial(lr_mat); // longitudinalRebar
  if (theLR_Material != 0)
  {
  ...
  if (be_length > 0.0)
  {
    SwimMaterial *theLRB_Material = theModel->getMaterial(lrb_mat); // longitudinalBoundaryElementRebar
  ...
  SwimMaterial *theTR_Material = theModel->getMaterial(tr_mat); // transverseRebar
  if (theTR_Material != 0)
  {
  ...
  SwimMaterial *theTRb_Material = theModel->getMaterial(trb_mat); // transverseRebar
  ...
  if (!lr_tag_ND)
  {
    int lr_name = ++SwimMaterial::numTagND;
    lr_tag_ND = lr_name;
  ...
  if (be_length > 0.0)
  {
    ...
    if (!lrb_tag_ND)
    {
      int lrb_name = ++SwimMaterial::numTagND;
      lrb_tag_ND = lrb_name;
    ...
    if (!trb_tag_ND)
    {
      int trb_name = ++SwimMaterial::numTagND;
      trb_tag_ND = trb_name;
  ...
  if (!tr_tag_ND)
  {
    int tr_name = ++SwimMaterial::numTagND;
    tr_tag_ND = tr_name;
  ...
  // write out concrete
  SwimMaterial *concrete = theModel->getMaterial(string("Concrete"));
  concrete->setParameters(theModel->beta,theModel->An,theModel->Ap,theModel->Bn);
...
void
ConcreteShearWall::initConcrete(double tbeta, double tAn, double tAp, double tBn)
{
  beta = tbeta;
  An = tAn;
  Ap = tAp;
...
void ConcreteShearWall::getMatsBIM()
{
    if (materialsArray != NULL)
    {
      ...
      for (int i = 0; i < numMat; i++)
      {
        ...
        const char *key;
        json_t *value;
        json_object_foreach(material, key, value) {
            const string trueKey(key);
            string trueValue;
            if(json_is_string(value))
                trueValue = string(json_string_value(value));
            ...
            tmpMap.insert(std::pair<string, string>(trueKey, trueValue));

        }


        matsBIM.push_back(tmpMap);
...
int ConcreteShearWall::readBIM(const char *event, const char *bim)
{
  ...
  materialsArray = json_object_get(properties, "materials");
  if (materialsArray != NULL)
  {
    ...
    for (int i = 0; i < numMat; i++)
    {
      ...
      theMaterials.insert(std::pair<string, SwimMaterial *>(string(name), theMaterial));
    }
  }
  ...
  if (wallSectionsArray != NULL)
  {
    ...
    for (int i = 0; i < numSection; i++)
    {
      ...
      ConcreteRectangularWallSection *theSection = 0;
      ...
      theWallSections.insert(std::pair<string, ConcreteRectangularWallSection *>(string(name), theSection));
    }
  }
  ...
  getMatsBIM();// calculate matsBIM
  calcProperties();// calculate properties

  return 0;
}

void ConcreteShearWall::calcProperties()
{
    map<string, double>::iterator itr;
    vector<double> floorElevstmp;
    for (itr = floors.begin()++; itr != floors.end(); ++itr) {
        floorElevstmp.push_back(itr->second);
    }
    for (int i = 1; i < floorElevstmp.size(); ++i) {
        floorHeights.push_back(floorElevstmp[i]-floorElevstmp[i-1]);
    }


  vector<double> loc1 = theModel->getCLineLoc("1");
  vector<double> loc2 = theModel->getCLineLoc("2");
  double cline1XLoc = loc1[0];
  double cline1YLoc = loc1[1];
  double cline2XLoc = loc2[0];
  double cline2YLoc = loc2[1];
...
int ConcreteShearWall::addNode(double x, double y, double z, json_t *theNodeArray)
{
  ...
  int result = -1;
  for (std::list<SwimNode>::iterator it = nodes.begin(); it != nodes.end(); ++it)
  {
    double xLoc = it->xLoc;
    double yLoc = it->yLoc;
    double zLoc = it->zLoc;
  ...
  {
    numNode++;
    SwimNode *newNode = new SwimNode(numNode, x, y, z);
...
{
  std::map<string, SwimMaterial *>::iterator it;
...
{
  std::map<string, ConcreteRectangularWallSection *>::iterator it;
